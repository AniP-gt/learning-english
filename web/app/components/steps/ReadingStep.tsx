...
type ReadingStepProps = {
  ...
  readingStatus: "idle" | "loading" | "ready";
};
...
export const ReadingStep = ({
  ...
  const isGeneratingReading = readingStatus === "loading";
  const generationButtonLabel = isGeneratingReading
    ? "Generating Reading…"
    : readingOutput
      ? "Regenerate Reading"
      : "Generate Reading";
  const generationButtonTitle = topicHeader
    ? "Generate or regenerate reading for the selected day"
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {!manualModeActive && (
            <button
              ...
              disabled={!topicHeader || isGeneratingReading}
              className="rounded-full border border-[#24283b] px-4 py-2 text-xs font-bold uppercase tracking-[0.35em] text-[#7aa2f7] transition hover:border-[#7aa2f7] hover:bg-[#7aa2f7]/10 disabled:opacity-40"
